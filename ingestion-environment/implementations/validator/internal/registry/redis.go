...
type SchemaRegistry interface {
	LoadSchema(deviceType, version, eventType string) (*jsonschema.Schema, error)
}
...
func (r *redisRegistry) LoadSchema(deviceType, version, eventType string) (*jsonschema.Schema, error) {
	k, err := r.key(deviceType, version, eventType)
	if err != nil {
		return nil, err
	}
	if v, ok := r.memCache.Load(k); ok {
	...
	if err != nil {
		if err == redis.Nil {
			return nil, fmt.Errorf("schema ausente no redis (%s)", k)
		}
		return nil, fmt.Errorf("falha ao buscar schema no redis (%s): %w", k, err)
	}
	if len(val) == 0 {
		return nil, fmt.Errorf("schema vazio no redis (%s)", k)
	}

	sch, cErr := compileSchema(val, k)
	if cErr != nil {
		return nil, cErr
	}
